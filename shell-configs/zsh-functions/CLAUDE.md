...
| `pane-beads` | Opens `beads-tui` in a new Zellij pane (direction: right). Finds the app under `plugins/claude-multiagent/scripts/beads-tui/` from the git root. Creates a shared `.beads-tui-venv` with `textual` if it doesn't exist. Passes extra args through to `beads_tui`. Must be run inside Zellij. |
| `pane-watch` | Opens `watch-dashboard` in a new Zellij pane (direction: down). Same venv and discovery logic as `pane-beads`. Passes extra args through to `watch_dashboard`. Must be run inside Zellij. |
| `panes`    | Runs both `pane-beads` and `pane-watch` in sequence, opening both dashboard panes at once. |
